...
server.tool(
  ...
  "Set the fill color of a node in Figma (TextNode or FrameNode). Supports RGBA colors with alpha/transparency. All color values are 0-1 range.",
  {
    ...
    g: import_zod.z.number().min(0).max(1).describe("Green component (0-1)"),
    b: import_zod.z.number().min(0).max(1).describe("Blue component (0-1)"),
    a: import_zod.z.number().min(0).max(1).optional().describe("Alpha/opacity component (0-1). 0=fully transparent, 1=fully opaque. Optional, defaults to 1.")
  },
...
server.tool(
  ...
  "Set the stroke color of a node in Figma. Supports RGBA colors with alpha/transparency. All color values are 0-1 range.",
  {
    ...
    b: import_zod.z.number().min(0).max(1).describe("Blue component (0-1)"),
    a: import_zod.z.number().min(0).max(1).optional().describe("Alpha/opacity component (0-1). 0=fully transparent, 1=fully opaque. Optional, defaults to 1."),
    weight: import_zod.z.number().positive().optional().describe("Stroke weight in pixels")
  },
  ...
  "Create a new paint/color style in Figma with single or multiple paint layers. IMPORTANT: Paint order matters! Paints are layered bottom-to-top: paints[0]=bottom layer (rendered first), paints[1]=middle, paints[n]=top layer. Supports solid colors (hardcoded or variable-bound), gradients, and image fills. Each layer can have its own opacity. Perfect for overlays, glassmorphism, or variable-based themes.",
  {
    ...
    paints: import_zod.z.array(
      import_zod.z.object({
        ...
        boundVariables: import_zod.z.object({
          color: import_zod.z.object({
            variableId: import_zod.z.string().describe("ID of the color variable to bind (from list_variables)")
        ...
        opacity: import_zod.z.number().min(0).max(1).optional().describe("Opacity (0-1), defaults to 1"),
        gradientStops: import_zod.z.array(
          import_zod.z.object({
            position: import_zod.z.number().min(0).max(1).describe("Position along gradient (0-1)"),
            color: import_zod.z.object({
        ...
        gradientTransform: import_zod.z.array(import_zod.z.array(import_zod.z.number())).optional().describe("Gradient transform matrix (optional, defaults to identity)"),
